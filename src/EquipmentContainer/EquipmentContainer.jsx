...
const EquipmentContainer = () => {
    ...
    const createNewEquip = async (newEquip) => {
        ...
        const apiResponse = await fetch("https://snowshelves2.herokuapp.com/", {
            method: "POST",
    ...
    const deleteEquip = async (idToDelete) => {
        try {
            const apiResponse = await fetch(`http://snowshelves2.herokuapp.com/equips/${idToDelete}`, {
                method: "DELETE"
            })
            const parsedResponse = await apiResponse.json()
            if (parsedResponse.success) {
    ...
    const getEquips = async () => {
        try {
            const equips = await fetch('http://snowshelves2.herokuapp.com/equips')
    ...
    const updateEquip = async (idToUpdate ,equipToUpdate) => {
        const apiResponse = await fetch(`http://snowshelves2.herokuapp.com//${idToUpdate}`, {
            method: "PUT",
